...
describe("envSchema", () => {
  ...
  it("should apply default values", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.BETTER_AUTH_URL).toBe("https://lyrikos.api.localhost");
      expect(result.data.PORT).toBe("4000");
      expect(result.data.HOST).toBe("0.0.0.0");
      expect(result.data.NODE_ENV).toBe("development");
      expect(result.data.FROM_EMAIL).toBe("noreply@lyrikos.example.com");
      expect(result.data.CORS_ORIGINS).toBe("https://lyrikos.web.localhost");
    }
  });
