...
describe("Auth Server Configuration", () => {
  ...
  it("should use lyrikos cookie prefix", () => {
    expect(auth.options.advanced?.cookiePrefix).toBe("lyrikos");
  });

  it("should have secure cookie settings when BETTER_AUTH_URL is HTTPS", () => {
    vi.stubEnv("BETTER_AUTH_URL", "https://lyrikos.example");
  ...
  it("should parse TRUSTED_ORIGINS env var as comma-separated list", () => {
    vi.stubEnv(
      "TRUSTED_ORIGINS",
      "https://app.lyrikos.example.com,https://api.lyrikos.example.com",
    );
    ...
    expect(envAuth.options.trustedOrigins).toEqual([
      "https://app.lyrikos.example.com",
      "https://api.lyrikos.example.com",
    ]);
  });
